...
class uhosting::profiles::nodejs {
  ...
  ensure_resource('nodejs::install', "nodejs-${::nodejs_stable_version}", { 'make_install' => false, 'version' => $::nodejs_stable_version })
  ensure_resource('file', '/usr/local/bin/node', { 
    'require' => Nodejs::Install[ "nodejs-${::nodejs_stable_version}" ],
    'ensure'  => "/usr/local/node/node-${::nodejs_stable_version}/bin/node",
  })
  ensure_resource('file', '/usr/local/bin/npm', { 
